...
pub fn list_migrations(
  page: Option(Int),
) -> Result(ScryfallList(CardMigration), client.ClientError) {
...
pub fn get_migration(id: Uuid) -> Result(CardMigration, client.ClientError) {
  request.new(http.Get, ["migrations", id])
  |> client.send(migrations.card_migration_schema())
}
